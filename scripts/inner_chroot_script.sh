...
# setup /etc/hosts, add argent as default hostname (required by Xfce)
sed -i "/^127.0.0.1/ s/localhost/localhost argent/" /etc/hosts
sed -i "/^::1/ s/localhost/localhost argent/" /etc/hosts
...
# Set Plymouth default theme, newer artwork has the argent theme
is_ply_argent=$(plymouth-set-default-theme --list | grep argent)
if [ -n "${is_ply_argent}" ]; then
	plymouth-set-default-theme argent
else
