...
ROOT_PART_MKFS_ARGS="${ROOT_PART_MKFS_ARGS:--L Argent}"
...
cleanup_loopbacks() {
	...
	[[ -n "${tmp_dir}" ]] && CHROOT_DIR="${tmp_dir}" "${ARGENT_MOLECULE_HOME}"/scripts/mmc_remaster_post.sh
}
...
CHROOT_DIR="${tmp_dir}" "${ARGENT_MOLECULE_HOME}"/scripts/remaster_pre.sh || exit 1
...
chroot_hook_include="${ARGENT_MOLECULE_HOME}/scripts/${chroot_hook_inc_name}"
...
CHROOT_DIR="${tmp_dir}" "${ARGENT_MOLECULE_HOME}"/scripts/mmc_remaster_post.sh
...
if [ -f "${tmp_dir}/etc/sudoers" ]; then
	echo "# added by Argent Molecule" >> "${tmp_dir}/etc/sudoers"
