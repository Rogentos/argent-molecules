...
create_embedded_grub_cfg() {
	...
	echo "echo Booting Argent" >> "${embedded_cfg}" || return 1
...
# Copy splash, this is in argent-artwork-grub, we expect to find it
...
shim_dir="${ARGENT_MOLECULE_HOME}"/boot/shim-uefi-secure-boot
...
# format (argent.cer), while sbsign requires a
# plain old text-based x509 certificate (argent.crt)
argent_der="${shim_dir}"/argent.cer
argent_cert="${shim_dir}"/argent.crt
...
if [ -f "${efi_x86_64_file}" ] || [ -f "${efi_i386_file}" ]; then
	...
	cp "${argent_der}" "${tmp_grub_dir}/SecureBoot/" || exit 1
